...
public class BidController {
    ...
    private Node buildCard(Bid b, FlowPane cards) {
        ...
        amount.setStyle("-fx-text-fill: #c0c0c0; -fx-font-size: 22; -fx-font-weight: bold;");
        ...
        Product pInfo = productMap.get(b.getProductId());
        boolean isProductActive = pInfo != null && !"sold".equals(pInfo.getStatus());
        
        if (session.isArtist() && isProductActive) {
            Button acceptBtn = createOutlineButton("🏆 Accepter l'offre");
            acceptBtn.setStyle("-fx-background-color: transparent; -fx-text-fill: #ff9800; -fx-border-color: #ff9800; -fx-border-radius: 6; -fx-background-radius: 6; -fx-padding: 6 14 6 14; -fx-cursor: hand;");
            acceptBtn.setOnMouseEntered(e -> acceptBtn.setStyle("-fx-background-color: #3a2a0a; -fx-text-fill: #ffb74d; -fx-border-color: #ffb74d; -fx-border-radius: 6; -fx-background-radius: 6; -fx-padding: 6 14 6 14; -fx-cursor: hand;"));
            acceptBtn.setOnMouseExited(e -> acceptBtn.setStyle("-fx-background-color: transparent; -fx-text-fill: #ff9800; -fx-border-color: #ff9800; -fx-border-radius: 6; -fx-background-radius: 6; -fx-padding: 6 14 6 14; -fx-cursor: hand;"));
            
            acceptBtn.setOnAction(e -> {
                Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "Accepter cette offre et clôturer l'enchère ?", ButtonType.YES, ButtonType.NO);
                alert.setHeaderText(null);
                alert.showAndWait().ifPresent(btn -> {
                    if (btn == ButtonType.YES) {
                        Order order = new Order();
                        order.setBuyerName(b.getBidderName());
                        order.setPrice(b.getAmount());
                        order.setOrderType("auction");
                        order.setProductId(b.getProductId());
                        orderService.add(order);
                        productService.markAsSold(b.getProductId());
                        
                        refreshCards(cards);
                    }
                });
            });
            buttons.getChildren().add(acceptBtn);
        }
