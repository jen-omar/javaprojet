...
public class LoginController {
    ...
    private void onSignup() {
        ...
        try {
            ...
            createdUser = userRepository.registerUser(username, email, password, firstName, lastName, phone);
        } catch (IllegalArgumentException ex) {
            setFeedback(ex.getMessage(), true);
            return;
        } catch (Exception ex) {
            setFeedback("Impossible de creer le compte: " + cleanError(ex.getMessage()), true);
            return;
        }
    ...
    private String cleanError(String message) {
        String value = Optional.ofNullable(message).orElse("").trim();
        return value.isBlank() ? "erreur base de donnees." : value;
    }
}
