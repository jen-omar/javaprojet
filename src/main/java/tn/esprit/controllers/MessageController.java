...
public class MessageController {
    ...
    public void onToggleRecord() {
        if (!isRecording) {
            ...
            if (currentAudioPath != null) {
                ...
                Message m = new Message(currentUser.getId(), currentBriefId, "Transcribing Voice...", finalAudioPath, LocalDateTime.now());
                messageDAO.add(m);
                loadMessages();
    ...
    public void onSendMessage() {
        ...
        Message m = new Message(currentUser.getId(), currentBriefId, text, null, LocalDateTime.now());
        messageDAO.add(m);
        messageInput.clear();
        loadMessages();

        // Check for AI Auto-reply (Bot Mode)
        User recipient = userDAO.getById(recipientId);
        if (recipient != null && recipient.isBotEnabled()) {
    ...
    public void onAiAutoReply() {
        ...
        String role = currentUser.getPrimaryRole();
        String persona;
        switch (role) {
            case "ROLE_AUTHOR":
    ...
    private void triggerBotAutoReply(User bot) {
        ...
        showAiStatus("🤖 " + bot.getUsername() + " is thinking…");

        String persona = "a helpful and concise AI assistant acting as '" + bot.getUsername()
        ...
        AiBridge.autoReplyAsync(messages, persona).thenAccept(reply -> {
            Platform.runLater(() -> {
                ...
                Message botMsg = new Message(bot.getId(), currentBriefId, reply, null, LocalDateTime.now());
                messageDAO.add(botMsg);
                loadMessages();
                showAiStatus("🤖 " + bot.getUsername() + " has responded.");
                fadeOutAiStatus(4);
            ...
            Platform.runLater(() -> {
                ...
                Message fallback = new Message(bot.getId(), currentBriefId,
    ...
    public void onOpenPortal() {
        ...
        kinshipPortal.openPortal(currentBriefId, currentUser.getUsername());
    }
